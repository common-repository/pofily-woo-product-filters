...
if ( sizeof( $list_label_items ) > 0 ) {
    ...
    >
        ...
        <div class="viwcpf_filter-wrap-items  <?php echo esc_attr( $toggle_class_name ); ?>">
			...
			?>
            <ul class="viwcpf_filter-items with-images viwcpf_filter-label  <?php echo esc_attr( $class_display_type ); ?> <?php echo esc_attr( $class_view_more ); ?>" <?php echo esc_attr( $data_view_more_limit ); ?>>
				<?php
				foreach ( $list_label_items as $label_item ) {
					...
					$get_images    = wp_get_attachment_image( $label_item['images'], 'thumbnail' );
					$btn_str_style = 'width: ' . $btn_width . 'px; border-radius: ' . $btn_border_radius . '; ';
					?>
